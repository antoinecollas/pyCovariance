...
def test_real_subspace_tau_UUH_RGD():
    ...
    feature = subspace_tau_UUH_RO(k, information_geometry=False)(p, N)
    ...
    # test with autodiff
    feature = subspace_tau_UUH_RO(
        k, information_geometry=False, autodiff=True)(p, N)
...
def test_real_estimation_tau_UUH():
    ...
    tau_BCD, U_BCD, _ = estimate_tau_UUH(X, k)
    ...
    tau_IG, U_IG, _ = estimate_tau_UUH_RO(X, k, information_geometry=True)
    assert tau_IG.shape == (N, 1)
    assert tau_IG.dtype == np.float64
    assert U_IG.shape == (p, k)
    assert U_IG.dtype == np.float64
    error = la.norm(U_IG@U_IG.T - U@U.T) / la.norm(U@U.T)
    assert error < 0.15

    tau_IG, U_IG, _ = estimate_tau_UUH_RO(
        X, k, information_geometry=True, solver='SGD', batch_size=100)
    assert tau_IG.shape == (N, 1)
    assert tau_IG.dtype == np.float64
    assert U_IG.shape == (p, k)
    assert U_IG.dtype == np.float64
    ...
    tau_BCD, U_BCD, _ = estimate_tau_UUH(X, k)
    tau_RGD, U_RGD, _ = estimate_tau_UUH_RO(X, k, information_geometry=False)
...
def test_real_tau_UUH_RGD():
    ...
    feature = tau_UUH_RO(k, information_geometry=False)(p, N)
...
def test_real_tau_UUH_IG():
    ...
    feature = tau_UUH_RO(k, information_geometry=True)(p, N)
