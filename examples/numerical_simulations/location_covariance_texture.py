...
def main(
    ...
    features_list = [
        location_covariance_texture_Gaussian_constrained_scatter(),
        location_covariance_texture_Tyler_constrained_scatter(),
        location_covariance_texture_constrained_scatter(iter_max=iter_max_RGD)
    ]
    ...
    for i, N in enumerate(iterator):
        ...
        def sample_fct():
            t = generate_textures_gamma_dist(N)
        ...
        true_parameters = [sigma, np.ones((N, 1))]
...
if __name__ == '__main__':
    main(nb_points=5, n_MC=1000, iter_max_RGD=3*int(1e4))
